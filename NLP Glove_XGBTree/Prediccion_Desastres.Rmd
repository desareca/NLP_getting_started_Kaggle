...
De estas variables se utilizan sólo **text** como característica para predecir **target**. Se observa que el dataset tiene **`r dim(dataDisaster)[1]`** observaciones, donde **`r round(mean(dataDisaster$target)*100,2)`%** de las observaciones son desastres reales.

```{r targetDist, include=T, warning=FALSE, fig.width=15, cache=TRUE}
...
Con lo anterior se genera un vocabulario con `r dim(vocab)[1]` términos, de los cuales a continuación se presentan los 40 más y menos frecuentes.
...
La representación selecciona las `r dim(wvOrder)[1]` palabras más comunes. Se observan varias relaciones de palabras con un contexto similar, como por ejemplo:
